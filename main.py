...
def make_creature(x, y, world: World):
    e = Entity().add_component(Movable()).add_component(Biochemistry()).add_component(Vision(3)).add_component(CommandQueue())
...
s.register_system(frozenset([CommandQueue]), decision_system, priority=100)

make_creature(1, 1, s.world)
make_creature(1, 11, s.world)
